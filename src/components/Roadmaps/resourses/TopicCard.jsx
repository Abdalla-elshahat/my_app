    ...
    <div className="flex flex-col items-center justify-center p-4 bg-gray-100 rounded-lg shadow-md mb-4 ">
      ...
      {topic.resources && topic.resources.length > 0 ? (
        topic.resources.map((res, index) => (
          <div
            key={index}
            className="bg-white shadow-lg rounded-2xl overflow-hidden max-w-3xl mx-auto mb-8 border border-gray-200 flex flex-row p-2 transition-transform transform hover:scale-105 duration-300 ease-in-out "
          >
            <img
              src={`${Domain}/${res.imageUrl}`}
              alt="Topic"
              className="w-1/2 h-72 object-cover rounded-2xl shadow-lg"
            />
