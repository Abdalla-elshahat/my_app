...
import { FaCheckCircle, FaExclamationCircle } from "react-icons/fa";
 export const getAllPosts = async (setData) => {
    try {
      ...
      const postsWithState = result?.data.map((post) => {
        const key =
          post.type === "Share"
            ? `share_${post.shareId}`
            : `post_${post.postId}`;
        const localData = likedPosts[key];

        return {
          ...post,
          isLiked: localData?.isLiked ?? post.isLiked,
          likesCount: localData?.likesCount ?? post.likesCount,
        };
      });

      setData(postsWithState);
    } catch (error) {
...
export   const toggleLike = async (post,setData) => {
 ...
 export  const handleUpdatePost = async (editPost,editTitle,editImages,setEditPost) => {
  ...
  export     const fetchCommentsByPostId = async (postId,setPostComments,setIsLoading) => {
        if (!postId) {
